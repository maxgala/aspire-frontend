...
class Quote extends Component {
  ...
  HandleRightArrowClick = (event) => {
    if (this.state.active < 6) {
      this.setState({
        current: Quotes[parseInt(this.state.active) + 1],
  ...
  HandleLeftArrowClick = (event) => {
    if (this.state.active > 0) {
      this.setState({
        current: Quotes[this.state.active - 1],
        active: this.state.active - 1,
      });
    } else if (this.state.active === 0) {
      this.setState({
        current: Quotes[this.state.numQuotes - 1],
  ...
  HandleSetClick = (event) => {
    this.setState({
      ...
      current: Quotes[event.target.getAttribute("data-image")],
    });
    this.timer();
  };
          ...
          >
            ...
            >
              <p className={classes.title}>
                {this.state.current.name} - {this.state.current.role} at{" "}
                {this.state.current.company}
              </p>
              <p className={classes.paragraph}>
                {this.state.current.testimonial}
              </p>
            </Grid>
            <Grid
              container
